...
-- Câu 24
SELECT HONV + ' ' + TENLOT + ' ' + TENNV AS 'DANH SACH NHAN VIEN CO THAN NHAN', COUNT(MATN) AS 'SO THANN HAN'
FROM NHANVIEN NV, THANNHAN TN
WHERE NV.MANV = TN.MANV
GROUP BY NV.MANV, HONV, TENLOT, TENNV
HAVING COUNT(MATN)>=2
...
-- Câu 26
SELECT HONV + ' ' + TENLOT + ' ' + TENNV AS 'DANH SACH TRUONG PHONG'
...
PB.TRPHG NOT IN (SELECT PB.TRPHG
FROM PHONGBAN PB, THANNHAN TN
...
SELECT HONV, TENLOT, TENNV 
FROM NHANVIEN NV
WHERE NV.MLUONG > (SELECT AVG(MLUONG) FROM NHANVIEN NV, PHONGBAN PB
...
FROM NHANVIEN NV, PHONGBAN PB
WHERE NV.MANV = PB.TRPHG AND
PB.MAPHG = (SELECT TOP 1 PB.MAPHG
                    FROM NHANVIEN NV, PHONGBAN PB
					WHERE NV.PHONG = PB.MAPHG
					GROUP BY PB.MAPHG
					ORDER BY COUNT (NV.PHONG) DESC
				  )
...
SELECT DISTINCT (NV.HONV + ' ' + NV.TENLOT + ' ' + NV.TENNV) AS 'Họ tên nhân viên', NV.DCHI
	...
	WHERE NV.PHONG = DA.MAPHG AND
		  NV.PHONG = DD.MAPHG AND
		  DA.DDIEM_DA IN (SELECT DA.DDIEM_DA
						    FROM DEAN DA
							) AND
		  DD.DIADIEM NOT LIKE DA.DDIEM_DA
